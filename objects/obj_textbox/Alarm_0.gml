...
else
{
    ...
    if (instance_exists(obj_control) && !layer_sequence_exists(obj_control.cur_seq_layer, sequence_to_show))
    {
        if (layer_exists(obj_control.cur_seq_layer))
        {
            // Create (play) the sequence
            layer_sequence_create(obj_control.cur_seq_layer, _cam_x, _cam_y, sequence_to_show);
